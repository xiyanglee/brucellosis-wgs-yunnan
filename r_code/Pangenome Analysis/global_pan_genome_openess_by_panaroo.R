...
#######################################
## Lineage 1 ~ 3 的基因开放性检测

length(metadata.phylo$assembly_accession[metadata.phylo$lineage_level_1 == '1'])
# [1] 657
curves_L1 <- calculate_pangenome_curves(datmat[metadata.phylo$assembly_accession[metadata.phylo$lineage_level_1 == '1'], ], break_step = 50, rep = 1000)
plotNewGenesReg(curves_L1$PAN_CURVE)

length(metadata.phylo$assembly_accession[metadata.phylo$lineage_level_1 == '2'])
# [1] 48
curves_L2 <- calculate_pangenome_curves(datmat[metadata.phylo$assembly_accession[metadata.phylo$lineage_level_1 == '2'], ], break_step = 5, rep = 1000)
plotNewGenesReg(curves_L2$PAN_CURVE)

length(metadata.phylo$assembly_accession[metadata.phylo$lineage_level_1 == '3'])
# [1] 140
curves_L3 <- calculate_pangenome_curves(datmat[metadata.phylo$assembly_accession[metadata.phylo$lineage_level_1 == '3'], ], break_step = 6, rep = 1000)
...
curves_L3_grop$PAN_CURVE <- curves_L3_grop$PAN_CURVE[5:(nrow(curves_L3_grop$PAN_CURVE) - 2), ]
...
print(label_L1)
# "Lineage 1: Adj R2 = 0.981 Intercept = 5.08 Slope = -1.43 P = 4.3e-10"
print(label_L2)
# "Lineage 2: Adj R2 = 0.257 Intercept = 3.23 Slope = -1.22 P = 0.0933"
print(label_L3)
# "Lineage 3: Adj R2 = 0.573 Intercept = 3.35 Slope = -1.21 P = 0.000261"
...
# ggsave('./plot/pan_genome_analysis/global_pan_genome_openess_by_panaroo.pdf', width = 5, height = 4)

ggplot() + 
  geom_ribbon(data = pred_df, aes(x = N, ymin = lwr, ymax = upr, group = group), fill = "black", alpha = 0.2, color = NA) + 
  geom_line(data = pred_df, aes(x = N, y = fit, group = group), color = "black", linewidth = 1, linetype = 2) + 
  # scale_color_manual(values = c("1" = "#38c2bf", "2" = "#c4901d", "3" = "#b86c90")) +
  # scale_fill_manual(values = c("1" = "#9DD0CF", "2" = "#D0A341", "3" = "#c9a7b7")) + 
  # ggnewscale::new_scale_color() + 
  geom_point(data = df_log, aes(x = N, y = New_genes, color = group), size = 3, alpha = 0.5) + 
  geom_point(data = df_log, aes(x = N, y = New_genes, color = group), fill = NA, size = 3, shape = 21, stroke = 1, alpha = 1) + 
  scale_color_manual(values = c("1" = "#38c2bf", "2" = "#c4901d", "3" = "#b86c90")) +
  # scale_fill_manual(values = c("1" = "#9DD0CF", "2" = "#D0A341", "3" = "#c9a7b7")) + 
  xlab("Log Number of genomes") +
  ylab("Log Number of new genes") +
  theme_bw(base_size = 14) + 
  annotation_logticks() + 
  coord_fixed(xlim = c(0.7, 3), ylim = c(0.7, 3)) + 
...
# ggsave('./plot/pan_genome_analysis/global_pan_genome_openess_by_panaroo_type2.pdf', width = 5, height = 4)

ggplot() + 
  geom_ribbon(data = pred_df, aes(x = N, ymin = lwr, ymax = upr, fill = group), alpha = 0.2, color = NA) + 
  geom_line(data = pred_df, aes(x = N, y = fit, color = group), linewidth = 1, linetype = 2) + 
  geom_point(data = df_log, aes(x = N, y = New_genes, color = group), size = 3, alpha = 0.5) + 
  geom_point(data = df_log, aes(x = N, y = New_genes, color = group), fill = NA, size = 3, shape = 21, stroke = 1, alpha = 1) + 
  scale_color_manual(values = c("1" = "#2c9969", "2" = "#c4901d", "3" = "#b86c90")) +
  scale_fill_manual(values = c("1" = "#9DD0CF", "2" = "#D0A341", "3" = "#c9a7b7")) +
  xlab("Log Number of genomes") +
  ylab("Log Number of new genes") +
  theme_bw(base_size = 14) + 
...
# ggsave('./plot/pan_genome_analysis/global_pan_genome_openess_by_panaroo_type3.pdf', width = 5, height = 4)
...
# 1     34 1.7152                           
# 2     32 1.6975  2  0.017705 0.1669  0.847



# save.image('./r_image/global_pan_genome_openess_by_panaroo.RData')
