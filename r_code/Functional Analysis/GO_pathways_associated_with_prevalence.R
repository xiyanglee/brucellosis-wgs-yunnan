...
# 1. 获取 GO term 名称并替换下划线为空格
go_names <- db.GO.emapper$TERM2NAME$NAME[
  db.GO.emapper$TERM2NAME$TERM %in% res$module_go_terms$go_terms[[4]]
]
go_names <- gsub("_", " ", go_names)

# 构建数据框
word_df <- data.frame(
  word = go_names,
  freq = table(factor(go_names, levels = unique(go_names)))
)

word_df <- as.data.frame(table(go_names))
colnames(word_df) <- c("word", "Freq")

# 可视化
ggplot(word_df, aes(label = word, size = Freq)) +
...
# 1. 获取 GO term 名称并替换下划线为空格
go_names <- db.GO.emapper$TERM2NAME$NAME[
  db.GO.emapper$TERM2NAME$TERM %in% res$module_go_terms$go_terms[[1]]
]
go_names <- gsub("_", " ", go_names)

# 2. 手动打散为单词
word_vector <- unlist(strsplit(go_names, "\\s+"))  # 按空格切分成词
word_vector <- tolower(word_vector)                # 转小写以归一化（可选）
word_vector <- word_vector[nchar(word_vector) > 2] # 去除长度≤2的短词（如"to", "on"）

# 3. 统计词频
word_df <- as.data.frame(table(word_vector))
colnames(word_df) <- c("word", "Freq")

# 4. 画词云
ggplot(word_df, aes(label = word, size = Freq)) +
  geom_text_wordcloud_area(color = "#12507b") +
  scale_size_area(max_size = 10) +
  theme_minimal()
