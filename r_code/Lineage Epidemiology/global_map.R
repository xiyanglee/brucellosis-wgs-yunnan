...
# 绘制地图和饼图
ggplot() + 
  geom_sf(data = world_coastline, fill = "gray95", color = 'black', linewidth = 0.5) + 
  # geom_sf(data = world, fill = "gray95", color = NA, size = 0.2) + 
  # geom_sf(data = world, aes(fill = sample_n), color = NA, size = 0.2, show.legend = TRUE) + 
  # scale_fill_viridis_c(option = "plasma", trans = "log10", na.value = "gray95", name = "Sample count") + 
  ...
  geom_scatterpie(
    ...
    color = "black",  linewidth = 0.25,   # 饼图边界
    alpha = 1
  ) + 
  ...
  coord_sf(xlim = c(-180, 180), ylim = c(-50, 90)) + 
...
# ggsave('./plot/global_map/global_map_v2.pdf', width = 10)
...
# ggsave('./plot/global_map/global_map_col2_v2.pdf', width = 12)
